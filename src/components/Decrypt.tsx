...
const Decrypt: React.FC<DropzoneProps> = ({ setInput, setEncryptionEnabled, setStringToDecrypt, decryptPassword, setDecryptPassword, setDecryptedText }: DropzoneProps) => {
    ...
    return (
        <div className='w-full h-full'>
            <div {...getRootProps()} className="flex items-center justify-center w-full h-[17em] mb-4 sm:mb-0 xs:mb-0">
                <label onChange={() => onDrop([...acceptedFiles])} tabIndex={0} htmlFor="dropzone-file" className="flex transition ease-in-out duration-300 flex-col items-center justify-center w-full h-full px-2 border-2 border-gray-300 border-dashed rounded-b-lg xs:rounded-none cursor-pointer bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500">
